...
int main()
{
	...
	while (1)
	{
		...
		case 1:
			...
			if (input_flight_vec(tab) == -1)
			{
				cout << "\nРейс с такой датой уже есть, вставка не удалась\n";
				break;
			}
			cout << "\nНовое табло:\n";
			output_table_vec(tab);
			break;
		case 2:
			cout << "\nВведите номер удаляемого рейса: ";
			cin >> nom;
			delete_flight_vec(nom, tab, arch_tab);
			cout << "\nНовое табло:\n";
			output_table_vec(tab);
			cout << "\nАрхив:\n";
			output_table_vec(arch_tab);
			break;
		case 3:
			cout << "\nВведите дату: ";
			cin >> nom;
			cout << "\n" << nom << " было задержано рейсов: " << count_delay_vec(nom,arch_tab, tab);
			break;
		case 4:
			cout << "\n";
			output_table_vec(tab);
